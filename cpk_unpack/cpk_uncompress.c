...
static inline uint16_t get_next_bits(FILE *infile, long long * const offset_p, uint8_t * const bit_pool_p, int * const bits_left_p, const int bit_count)
{
    uint16_t out_bits = 0;
...
long long uncompress(FILE *infile, long long offset, long long input_size, FILE *outfile)
{
    ...
    const long long uncompressed_size =
        get_32_le_seek(offset+0x08, infile);

    const long long uncompressed_header_offset =
    ...
    const long long input_end = offset + input_size - 0x100 - 1;
    long long input_offset = input_end;
    const long long output_end = 0x100 + uncompressed_size - 1;
    uint8_t bit_pool = 0;
    int bits_left = 0;
    long long bytes_output = 0;

    while ( bytes_output < uncompressed_size )
    {
        if (GET_NEXT_BITS(1))
        {
            long long backreference_offset =
                output_end-bytes_output+GET_NEXT_BITS(13)+3;
            long long backreference_length = 3;
