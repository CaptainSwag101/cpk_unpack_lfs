...
void analyze_CPK(FILE *infile, const char *base_name, long long file_length);
...
int main(int argc, char **argv)
{
    ...
    /* get file size */
    CHECK_ERRNO(_fseeki64(infile, 0 , SEEK_END) != 0, "fseek");
    long long file_length = _ftelli64(infile);
...
void analyze_CPK(FILE *infile, const char *base_name, long long file_length)
{
    const long long CpkHeader_offset = 0x0;
    ...
    /* get TOC offset */
    long long toc_offset = query_utf_8byte(infile, CpkHeader_offset+0x10, 0, "TocOffset");

    /* get content offset */
    long long content_offset = query_utf_8byte(infile, CpkHeader_offset+0x10, 0, "ContentOffset");

    /* get file count from CpkHeader */
    long long CpkHeader_count = query_utf_4byte(infile, CpkHeader_offset+0x10, 0, "Files");
    ...
    for (int i = 0; i < toc_entries; i++)
    {
        ...
        /* get file size */
        long long file_size = query_utf_4byte(infile, toc_offset+0x10, i,
                "FileSize");

        /* get extract size */
        long long extract_size = query_utf_4byte(infile, toc_offset+0x10, i,
                "ExtractSize");
        ...
        CHECK_ERROR( file_offset_raw > UINT64_MAX, "File offset too large, will be unable to seek" );
        long long file_offset = file_offset_raw;

        printf("%s/%s 0x%llx %lld\n",
                dir_name, file_name, (unsigned long long)file_offset, file_size);
